...
from pydantic i            # Try to search for actual prices on e-commerce sites
...
📝 **Note:** Prices are estimates based on AI analysis. For exact pricing, visit the retailer websites directly."""ld, AnyUrl
...
class PuchAIImageHandler:
    ...
    async def fetch_image_by_id(image_id: str) -> Optional[str]:
        ...
        # Try multiple possible Puch AI endpoints
        possible_endpoints = [
                    ...
                    if response.status_code == 200:
                        ...
                        if content_type.startswith("image/"):
                            ...
                            try:
                                ...
                                if DEBUG:
                                    print(f"Debug: JSON response keys: {json_data.keys()}")
                                
                                # Check various possible field names
                                for field in ["image_data", "data", "base64", "content", "image"]:
                                    if field in json_data and json_data[field]:
                                        if DEBUG:
                                            print(f"Debug: Found image data in field: {field}")
                                        return json_data[field]
                                
                                # Check for URL field
                                for field in ["url", "image_url", "file_url", "download_url"]:
                                    if field in json_data and json_data[field]:
                                        if DEBUG:
                                            print(f"Debug: Found image URL in field: {field}")
                                        # Fetch from the URL
                                        img_response = await client.get(json_data[field], timeout=10)
    ...
    async def create_mock_analysis(image_id: str) -> Dict[str, Any]:
        ...
        # Expanded mock products with better variety based on image_id patterns
        mock_products = [
            {
                ...
                "key_features": ["15.6-inch Display", "Intel Core i5", "8GB RAM", "256GB SSD"],
            ...
            },
            {
                ...
                "category": "laptop", 
                "model": "Budget Laptop",
                "key_features": ["15.6-inch HD Display", "AMD Ryzen 5", "8GB RAM"],
            ...
            },
            {
                ...
                "brand": "HP",
                "category": "laptop",
                "model": "2-in-1 Convertible",
                "key_features": ["14-inch Touchscreen", "Intel Core i3", "Convertible Design"],
                "estimated_price_range": "$499-699",
                "confidence": "Medium (Demo Analysis)"
            },
            {
                ...
                "key_features": ["15.6-inch FHD", "AMD Ryzen 7", "Fingerprint Reader"],
            ...
            },
            {
                ...
                "category": "smartphone",
                "model": "A3108",
                "key_features": ["Pro Camera System", "Titanium Design", "A17 Pro Chip"],
            ...
            },
            {
                ...
                "category": "smartphone",
                "model": "Flagship",
                "key_features": ["Dynamic AMOLED", "AI Camera", "5G Connectivity"],
                "estimated_price_range": "$799-999",
                "confidence": "Medium (Demo Analysis)"
            }
        ]
        
        # Select based on image_id hash for consistency
        import hashlib
        hash_val = int(hashlib.md5(image_id.encode()).hexdigest(), 16)
        selected_product = mock_products[hash_val % len(mock_products)]
        
        # Add a note that this is demo analysis
        selected_product["demo_note"] = f"⚠️ This is demo analysis for image ID: {image_id[:8]}..."
